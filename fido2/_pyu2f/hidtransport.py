...
class UsbHidTransport(object):
  ...
  """
    ...
    def __init__(self, hid_device, read_timeout_secs=3.0):
        ...
        if in_size != out_size:
            raise OSError("unsupported device with different in/out packet sizes.")
        if in_size == 0:
    ...
    def InternalSend(self, cmd, payload):
        ...
        first_packet = UsbHidTransport.InitPacket(
            self.packet_size, self.cid, cmd, len(payload), first_frame
        )
        ...
        while length_to_send > 0:
            ...
            next_packet = UsbHidTransport.ContPacket(
                self.packet_size, self.cid, seq, next_frame
            )
