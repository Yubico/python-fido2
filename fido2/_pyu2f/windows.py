...
def OpenDevice(path, enum=False):
    ...
    h = kernel32.CreateFileA(
        path, desired_access, share_mode, None, OPEN_EXISTING, 0, None
    )
...
class WindowsHidDevice(base.HidDevice):
    ...
    def Enumerate():
        ...
        interface_info.cbSize = ctypes.sizeof(
            DeviceInterfaceData
        ...
        while True:
            result = setupapi.SetupDiEnumDeviceInterfaces(
                devices, 0, ctypes.byref(hid_guid), index, ctypes.byref(interface_info)
            )
            index += 1
            if not result:
                break
    ...
    def Write(self, packet):
        ...
        ret = kernel32.WriteFile(
            self.dev, out, len(out), ctypes.byref(num_written), None
        )
